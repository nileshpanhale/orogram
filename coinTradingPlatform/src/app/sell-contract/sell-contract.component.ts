...
@Component({
  ...
  styleUrls: ['./sell-contract.component.scss'],
  providers: [SearchPipe]
})
export class SellContractComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('notifications')) {
      ...
      this.sharedService.updateNotification({ type: 'CREATE_CONTRACT', userId: user.userId, value: -(parseInt(this.notifications['openContractSell'])), tradeType: 'BUY' });
  ...
  set(i: number) {
    ...
    //console.log('In sell:',this.accountCalculatedCoin);
    console.log('In sell contract coins:', this.chosenContainer.coins);
  ...
  amountEuro(coin: any) {
    return (this.bitoroPrice * coin).toFixed(8)
  }
  ...
  getLink(values: any) {
    ...
    return (values.replace(urlRegex, `<a href = "$1">$1</a>`));
