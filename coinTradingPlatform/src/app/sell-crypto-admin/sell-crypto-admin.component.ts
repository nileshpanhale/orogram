...
@Component({
  ...
  templateUrl: './sell-crypto-admin.component.html',
  styleUrls: ['./sell-crypto-admin.component.scss']
})
export class SellcryptoAdminComponent implements OnInit {
  [x: string]: any;
  ...
  columns = ['Date', 'Description', 'Txn Type', 'Orogram Amount', 'Mode of Payment', 'Account used', 'Status', 'Action']
  ...
  constructor(
    ...
    this.goldForm = this.formBuilder.group({
      carat: ['', Validators.compose([Validators.required, Validators.minLength(1)])],
  ...
  ngOnInit() {
    ...
    console.log("Fetched Transactions : ", this.getTransactions());
    ...
    this.dataService.getAccountDetails().subscribe(
      (data: any) => {
        console.log("console is data", data);
  ...
  getTransactions() {
    this.loading = true;
    this.dataService.getPurchaseCoinAdmin().pipe((finalize(() => { this.loading = false }))).subscribe(res => {
  ...
  submitForm() {
    ...
    if (this.purchaseCoinForm.value.type == 'btc') {
      ...
      payload2['coins'] = this.btcForm.value.amount
    }
    else if (this.purchaseCoinForm.value.type == 'gold') {
      delete payload2['bankAccount'];
      ...
      delete payload2['goldDetails'];
      delete payload2['cash'];
      payload2['coins'] = this.wireForm.value.amount
    }
    else {
      delete payload2['receiverAccount'];
      delete payload2['goldDetails'];
      delete payload2['bankAccount'];
      payload2['coins'] = this.cashForm.value.amount
    }

    this.dataService.purchaseCoinAdmin(payload2).subscribe(res => {
      ...
      this.socket.emit('usernotification', { type: 'SELL' });
    }, (error) => {
  ...
  change(i: any) {
    this.loading = true;
    this.dataService.changeStatusTransactionUser(this.transactions[i]._id, { 'status': this.action }).subscribe(
      data => {
  ...
  perPageChange() {
    this.loading = true;
    this.dataService.getPurchaseCoinAdmin(this.perPage, 1, this.status).pipe((finalize(() => { this.loading = false }))).subscribe(res => {
      this.generatePageNumbers(this.count);
      this.currentPage = 1;
      this.transactions = res['transactions'];
    });
  ...
  pageChange(value?: any) {
    ...
    this.dataService.getPurchaseCoinAdmin(this.perPage, this.currentPage, this.status).pipe((finalize(() => { this.loading = false }))).subscribe(res => {
      this.generatePageNumbers(this.count);
      this.transactions = res['transactions'];
    });
  }
  ...
  showWireDetails(item: any) {
    ...
    some += `<div class="topModel" style="text-align: left !important;"><div class='row'><div class='col-6'><b>Invoice Number:</b></div><div class="col-6">` + item.transactionId
      + "</div></div><br><div class='row'><div class='col-6'><b>Orogram Amount:</b></div><div class='col-6'>" + item.coins
  ...
  showCashDetails(item: any) {
    ...
    // var some = `<div class="topModel" style="text-align: left !important;"><div><b>Account Number:</b>`+item.receiverAccount+"</div><br><div><b>Amount:</b>"+item.amount+"</div><br><div><b>Currency:</b>"+item.currency+"</div><br><div><b>Payment Mode:</b>"+item.type+"</div><br><div><b>Transaction Type:</b>"+item.transactionType+"</div><br><div><b>Date:</b>"+date+"</div><br><div><b>Status:</b>"+item.status+"</div></div>"
    some += `<div class="topModel" style="text-align: left !important;"><div><b>Invoice Number:</b>` + item.transactionId
      + "<div><b>Orogram Amount:</b>" + item.coins
  ...
  showGoldDetails(item: any) {
    ...
    // var some = `<div class="topModel" style="text-align: left !important;"><div><b>Account Number:</b>`+item.receiverAccount+"</div><br><div><b>Amount:</b>"+item.amount+"</div><br><div><b>Currency:</b>"+item.currency+"</div><br><div><b>Payment Mode:</b>"+item.type+"</div><br><div><b>Transaction Type:</b>"+item.transactionType+"</div><br><div><b>Date:</b>"+date+"</div><br><div><b>Status:</b>"+item.status+"</div></div>"
    some += `<div class="topModel" style="text-align: left !important;"><div><b>Carat:</b>` + item.goldDetails.carat
      + "</div><br><div><b>Payment Mode:</b>" + item.type
      ...
      + "</div><br><div><b>Invoice Number:</b>" + item.transactionId + "</div></div>"
    swal({
      title: '<strong> Purchase Details </strong>',
      html: some,
      width: 800,
