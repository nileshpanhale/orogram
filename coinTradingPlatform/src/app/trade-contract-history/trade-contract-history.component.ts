...
@Component({
  ...
  templateUrl: './trade-contract-history.component.html',
  styleUrls: ['./trade-contract-history.component.scss']
})
export class TradeContractHistoryComponent implements OnInit {
      ...
      if (this.uploader.queue.length == this.cnt) {
        ...
        this.dataService.changeStatusTransactionContract(this.container[this.savecontainerid]._id, payload).subscribe(
          (data: any) => {
            ...
            this.socket.emit('usernotification', { type: 'CONTRACT_HISTORY', userId: user.userId, historyUserId: this.container[this.savecontainerid].creator });
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('historyNotifications')) {
      ...
      let user: any = this.userService.user;
      this.sharedService.updateNotification({ type: 'CONTRACT_HISTORY', userId: user.userId, value: -(parseInt(this.historyNotifications['contractHistory'])), historyUserId: user._id });
    ...
    this.socket.on('message', (msg: any) => {
      ...
      let convertToArray = [];
      if (msg.transactionId != this.transactionId) {
  ...
  confirm_final() {
    this.loading = true;
    console.log("trade contract history loader");
    $('#confirm').modal('hide');
