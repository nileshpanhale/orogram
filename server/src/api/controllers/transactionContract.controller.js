...
const landNFTAPI = require('../../../../land-contract/contractAPI');
const landContractTRX = require('../models/landContract.model'); 
...
exports.mintLandNFT = async (req, res) => {
    ...
    try {
        ...
        let landPrivateContract = new landContractTRX(req.body);
        let contractdate = toNumber(req.body.contractdate);
        landPrivateContract._id = await getNextSequenceValue("product_id");
        landPrivateContract.coins = req.body.tradeCoin;
        landPrivateContract.sender = req.body.sender;
        landPrivateContract.receiver = req.body.receiver;
        landPrivateContract.isContract = req.body.isContract;
        landPrivateContract.contractDays = parseInt(contractdate);

        landPrivateContract.creator = req.body.receiver;
        user = req.body.receiver;
        landPrivateContract.creatorEmail = req.body.creatorEmail;

        if (req.body.receiver) {
            type = 'buy';
            landPrivateContract['transactionType'] = 'buy';
            let receiver = await User.findById(req.body.receiver);
            landPrivateContract['receiverWallet'] = receiver.account.address;
            ethPriv = crypr.decrypt(receiver.ethPrivateKey);
            ethPub = receiver.ethPublicKey;
        }
        
        let imageUrl = "assets/images/1686851628507_transaction.pdf";
        let contractText = "Land purchase private contract";

        let data = await contractService.createContact(landPrivateContract._id, parseInt(contractdate), contractText, ethPub, ethPriv, imageUrl);
        console.log("////////////////////////////////////////////////////");
        console.log("Contract creation data : ", data);

        landPrivateContract['contractStatus'][0] = {
            contractHash: data.transactionHash,
            contractStatus: 'created'
        }

        const landContract = await landPrivateContract.save();
        
        
        // let buyer = await User.find({email:landNFT.owner});
        
        let buyLand = await landNFTAPI.mintNFT(ethPub, newLand._id);

        console.log("contract response : ", landContract);
        console.log("database response : ", newLand);

        return res.status(httpStatus.OK).send(buyLand.status);
...
exports.getLandTradesHistory = async (req, res, next) => {
    ...
    try {
        ...
        let count;
        const transactions = await landContractTRX.find(queryObj['query']).sort({ createdAt: -1 });
        count = transactions.length;
...
exports.updateLandTransaction = async (req, res, next) => {
    try {

        let transaction = await landContractTRX.findById(req.params.id);
        let hold = 0;
 ...
 * Admin will confirm the disputed transactions
 */
exports.confirmLandTransaction = async (req, res, next) => {
    try {
        ...
        if (req.body.status.toLowerCase() === 'cancel') {
            ...
            //removing holded coins form admin account on Cancel --------------------
            let updatedTransaction = await landContractTRX.findByIdAndUpdate(req.params.id, { status: req.body.status });
        ...
        if (req.body.status && req.body.status == 'confirm') {
            ...
            //buyer account details
            let sender = await User.findById(transaction.receiver);
            console.log("Buyer account details : ", sender);
            //seller account details
            // let receiver = await User.findById(transaction.sender);
            //system account details
            // let admin1 = await User.findOne({ role: 'admin1' });
            ...
            //sending land purchase contract coins to admins account from buyer account
            try {
                contractCoin = await index.Transfer(sender.ethPublicKey, crypr.decrypt(sender.ethPrivateKey), admin.ethPublicKey, coins);
                console.log("///////////////////////////////////////////");
                console.log("Land Purchase coins transferred.");
                let TRX = new blkTRX({
                    type: 'coinTransfer',
                    status: 'holding',
                    coins: coins,
            ...
            if (contractCoin) {
                ...
                let updatedTransaction = await landContractTRX.findByIdAndUpdate(req.params.id, { status: req.body.status, transactionId: contractCoin.transactionHash, coins: coins });
...
exports.readMap = async (req, res) => {
    try {
        let mapData = await nftDB.find().sort({ timestamp: -1 });
