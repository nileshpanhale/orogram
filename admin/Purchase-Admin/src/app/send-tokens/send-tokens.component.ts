...
export class SendTokensComponent implements OnInit {
  ...
  getAdminTx(){
    this.userService.getAdminTx(this.perPage, this.currentPage).subscribe(
      (data:any) => {
        console.log("Fetched data of admin trx : ", data);
  ...
  onSubmit(values:any):void{
    ...
    if(this.form.valid){
      let payload = {
        // 'coins':this.isCorrectValue(values.noOfcoins, true,0),
        'coins':values.noOfcoins,
  ...
  intToDecimal (input:any){
   ...
   pageChange(value?: any) {
    ...
    this.userService.getAdminTx(this.perPage, this.currentPage, this.status).subscribe( res => {
      this.generatePageNumbers( this.count  );
   ...
   perPageChange() {
    this.userService.getAdminTx(this.perPage,1, this.status).subscribe( res => {
      this.generatePageNumbers( this.count  );
      this.currentPage = 1;
      this.container = res['transactions'];
      // this.setActive();
