...
export class ContractsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.socket.on('getAllChatData', (msg: any) => {
      ...
      if (this.fetchingInitially) {
        this.fetchingInitially = false;
    ...
    this.socket.on('showAdminNotification', (msg: any) => {

      // this.chatroomTransactions = msg;

      console.log("All sd,jfhsfjhdb Data ", msg)
  ...
  modifyContainer(){
    for (let i = 0; i < this.container.length; i++) {
      ...
      for (let j = 0; j < this.chatroomTransactions.length; j++) {
        // console.log(this.chatroomTransactions[j].transactionId == "786");

        if (this.container[i]._id == this.chatroomTransactions[j].transactionId) {

          if (this.container[i]._id == "521") {
            console.log('521 found');
          ...
          //condition added to Hide / Show Bell icon -----------------------
          if ((countUnread != 0) || this.container[i]['isNotification']) {
  ...
  getTradeTx(perPage?, currentPage?, status?) {
    this.userService.getContractTx(perPage, currentPage, this.filterPayload, status).subscribe(
      data => {
        console.log("data of getTradeTx ", data);
        ...
        for (let i = 0; i < this.container.length; i++) {
          ...
          for (let j = 0; j < this.chatroomTransactions.length; j++) {
            ...
            if (this.container[i]._id == this.chatroomTransactions[j].transactionId) {
              ...
              // console.log("Transaction Matched",countUnread);
              //condition added to Hide / Show Bell icon -----------------------
              if (((countUnread != 0) || this.container[i]['isNotification']) && (this.chatroomTransactions[j]['includeAdmin'])) {
